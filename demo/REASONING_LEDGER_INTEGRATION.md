...
reasoning_result = self.reasoning_ledger.store_reasoning_simple(
    ...
    llm_provider="anthropic",
    llm_model="claude-sonnet-4.5-20250514",
    input_triggers=[trigger_data['trigger_id']]
)
...
### 2. **Add Step-by-Step Reasoning Tracking** (Throughout `_process_news_trigger`)
...
def _process_news_trigger(self, trigger_data: Dict) -> SignalTrigger:
    ...
    reasoning_steps.append(
        ReasoningLedgerHelper.create_reasoning_step(
            ...
            input_data={"walrus_blob_id": trigger_data['walrus_blob_id']},
...
**Current:** Agent A relies on `publisher.publish_signal_trigger()` to store reasoning
...
**Add to Agent A:**
```python
def verify_past_reasoning(self, signal_trigger_id: str) -> bool:
    """
    Verify the integrity of a past reasoning trace.

    Args:
        signal_trigger_id: Signal trigger to verify

    Returns:
        True if reasoning trace is valid
    """
    # Get signal from registry
    signal_data = self.registry.get_trigger(signal_trigger_id)
...
3. Add `reasoning_hash` to TriggerRegistry

### Phase 2: Step Tracking
4. Add `reasoning_steps` list to `_process_news_trigger`
